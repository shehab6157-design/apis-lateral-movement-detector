...
```bash
pip install -r requirements.txt        # or: pip install pytest fastapi uvicorn scapy

# 1. build a baseline from known-clean traffic
python baseline.py clean_traffic.txt

# 2. run detection
python detector.py traffic.txt

# 3. identity side
python auth_baseline.py clean_auth.txt
python auth_pipeline.py auth_events.txt

# 4. AI-agent side
python claude_code_adapter.py agent_events.txt
python agent_baseline.py agent_events.txt
python agent_pipeline.py agent_events.txt

# 5. verify the audit trail has not been altered
python verify_audit_trail.py

# 6. run as a service
python -m uvicorn api_server:app --reload      # http://127.0.0.1:8000/docs
```
